...
parser = argparse.ArgumentParser(
    description="Takes output file, logfile config, secret"
)
parser.add_argument(
    "-i", "--indir", help="Input directory base path", required=True
)
parser.add_argument("-g", "--gold", help="Gold data directory", required=True)
parser.add_argument(
    "-s", "--sub", help="Submission data directory", required=True
)
parser.add_argument(
    "-m",
    "--mode",
    help="Mode to run scoring: overall, class, doc, classdoc (both), sub, or classsub; default is overall.",
    default="overall",
)
parser.add_argument(
    "--skip", help="input file of files to skip for debugging, one id per line."
)
parser.add_argument(
    "-v", "--val", help="Validate submission only.", action="store_true"
)
parser.add_argument(
    "-l",
    "--limit",
    help="Limit gold data loaded to files also in submission.",
    action="store_true",
)
    ...
    def validate(self, field, row={}):
        ...
        if expectedLen != textLen and (field or not self.empty_ok):
            ...
            raise ValidationException(
                "'{}' length {} does not match extpected length /{}/".format(
                    field, textLen, expectedLen
                )
            )
...
class JsonValidator(Validator):
    ...
    def validate(self, field, row={}):
        if (not field) and (not self.empty_ok):
            self.failures.add(field)
            raise ValidationException("'{}' is empty".format(field))
        elif field != "":
            # print(field)
            try:
                ...
                if row["annotType"] == "Quantity":
                    if not all(
                        k in ["mods", "unit"] for k in list(data.keys())
                    ):
                        self.failures.add(field)
                        raise ValidationException(
                            "'{}' has invalid key".format(field)
...
for sfn in os.listdir(args.indir + args.sub):
    if sfn not in skip and sfn.endswith(".tsv"):
        ...
        # print(sfn)
        truth = Vlad(
            source=LocalFile(args.indir + args.sub + sfn),
...
for annotType in [
    ...
    print(
        ...
        + annotType
        + ": "
        + str(len(gold.loc[gold["annotType"] == annotType].index))
    )
print("")
for annotType in [
    "Quantity",
    ...
    print(
        ...
        + annotType
        + ": "
        + str(len(sub.loc[sub["annotType"] == annotType].index))
    )
print("")
...
quantityMatches["EM"] = quantityMatches.apply(
    lambda x: 1.0 if (x.aStart == x.gStart and x.aEnd == x.gEnd) else 0, axis=1
)
...
def calcF1(row):
    ...
    overlapText = row.aText[
        overlapSubStrStart : overlapSubStrStart + overlapSubStrLen
    ]
...
quantityMatches["maxF1"] = quantityMatches.groupby(["docId", "annotId"])[
    "F1"
].transform("max")
...
annotSetAlignments = quantityMatches[["docId", "annotSet", "gAnnotSet"]].rename(
    columns={"gAnnotSet": "matchAnnotSet"}
)
annotSetAlignmentsDict = (
    annotSetAlignments.groupby("docId")
    .apply(lambda x: dict(zip(x.annotSet, x.matchAnnotSet)))
    .to_dict()
)

# Update submission data with corresponding gold annotSet if applicable.
sub["gAnnotSet"] = None
sub["gAnnotSet"] = sub.apply(
    lambda x: annotSetAlignmentsDict[x.docId][x.annotSet]
    if x.docId in annotSetAlignmentsDict
    and x.annotSet in annotSetAlignmentsDict[x.docId]
    else None,
    axis=1,
)
...
goldUnits["json"] = goldUnits.apply(
    lambda x: json.loads(str(x.other)) if str(x.other) != "nan" else "", axis=1
)
goldUnits["unit"] = goldUnits.apply(
    lambda x: x.json["unit"] if "unit" in x.json.keys() else "", axis=1
)
goldUnits = goldUnits[goldUnits.unit != ""][
    [
        "docId",
        "annotSet",
        "annotType",
...
subUnits = sub[
    sub["annotType"] == "Quantity"
].copy()  # and sub.other.notnull()]
subUnits = subUnits[subUnits.other.notnull()]
subUnits["json"] = None
subUnits["unit"] = None
subUnits["json"] = subUnits.apply(
    lambda x: json.loads(str(x.other)) if str(x.other) != "nan" else "", axis=1
)
subUnits["unit"] = subUnits.apply(
    lambda x: x.json["unit"] if "unit" in x.json.keys() else "", axis=1
)
# print(subUnits.head())
subUnits = subUnits[subUnits.unit != ""][
    [
        "docId",
        "annotSet",
        "gAnnotSet",
...
unitMatches["EM"] = unitMatches.apply(
    lambda x: 1.0 if (x.sUnit == x.gUnit) else 0, axis=1
)
unitMatches["F1"] = unitMatches.apply(
    lambda x: 1.0 if (x.sUnit == x.gUnit) else 0, axis=1
)
...
entityMatches["EM"] = entityMatches.apply(
    lambda x: 1.0 if (x.aStart == x.gStart and x.aEnd == x.gEnd) else 0, axis=1
)
entityMatches["F1"] = entityMatches.apply(lambda x: calcF1(x), axis=1)
entityMatches["maxF1"] = entityMatches.groupby(["docId", "annotId"])[
    "F1"
].transform("max")
...
propertyMatches["EM"] = propertyMatches.apply(
    lambda x: 1.0 if (x.aStart == x.gStart and x.aEnd == x.gEnd) else 0, axis=1
)
propertyMatches["F1"] = propertyMatches.apply(lambda x: calcF1(x), axis=1)
propertyMatches["maxF1"] = propertyMatches.groupby(["docId", "annotId"])[
    "F1"
].transform("max")
...
qualifierMatches["EM"] = qualifierMatches.apply(
    lambda x: 1.0 if (x.aStart == x.gStart and x.aEnd == x.gEnd) else 0, axis=1
)
qualifierMatches["F1"] = qualifierMatches.apply(lambda x: calcF1(x), axis=1)
qualifierMatches["maxF1"] = qualifierMatches.groupby(["docId", "annotId"])[
    "F1"
].transform("max")
...
tmpgRels = gold.loc[
    gold["annotType"].isin(["MeasuredEntity", "MeasuredProperty", "Qualifier"])
].copy()
tmpgRels["json"] = tmpgRels.apply(
    lambda x: json.loads(str(x.other)) if str(x.other) != "nan" else "", axis=1
)
...
goldRels = tmpgRels[
    ["docId", "annotSet", "annotType", "relType", "src", "target"]
]

tmpsRels = sub.loc[
    sub["annotType"].isin(["MeasuredEntity", "MeasuredProperty", "Qualifier"])
].copy()
tmpsRels["json"] = None
tmpsRels["relType"] = None
tmpsRels["target"] = None
tmpsRels["src"] = None
tmpsRels["json"] = tmpsRels.apply(
    lambda x: json.loads(str(x.other)) if str(x.other) != "nan" else "", axis=1
)
...
subRels = tmpsRels[
    ["docId", "annotSet", "gAnnotSet", "annotType", "relType", "src", "target"]
]
...
destMatch = pd.concat(
    [entityMatches, propertyMatches, quantityMatches], ignore_index=True
)
...
goldMods["json"] = goldMods.apply(
    lambda x: json.loads(str(x.other)) if str(x.other) != "nan" else "", axis=1
)
goldMods["mods"] = goldMods.apply(
    lambda x: x.json["mods"] if "mods" in x.json.keys() else "", axis=1
)

goldMods = goldMods.explode("mods")
goldMods = goldMods[goldMods.mods != ""][
    [
        "docId",
        "annotSet",
        "annotType",
...
subMods["json"] = None
subMods["mods"] = None
subMods["json"] = subMods.apply(
    lambda x: json.loads(str(x.other)) if str(x.other) != "nan" else "", axis=1
)
subMods["mods"] = subMods.apply(
    lambda x: x.json["mods"] if "mods" in x.json.keys() else "", axis=1
)

subMods = subMods.explode("mods")
subMods = subMods[subMods.mods != ""][
    [
        "docId",
        "annotSet",
        "gAnnotSet",
...
quantityMatches["annotType"] = quantityMatches.apply(
    lambda x: "Quantity", axis=1
)
...
goldOnlyQuants["matchType"] = goldOnlyQuants.apply(
    lambda x: "Gold only", axis=1
)
...
subOnlyEntities["matchType"] = subOnlyEntities.apply(
    lambda x: "Sub only", axis=1
)
...
goldOnlyEntities["matchType"] = goldOnlyEntities.apply(
    lambda x: "Gold only", axis=1
)
...
subOnlyProperties["matchType"] = subOnlyProperties.apply(
    lambda x: "Sub only", axis=1
)
...
goldOnlyProperties["matchType"] = goldOnlyProperties.apply(
    lambda x: "Gold only", axis=1
)

qualifierMatches["matchType"] = qualifierMatches.apply(
    lambda x: "Match", axis=1
)
...
subOnlyQualifiers["matchType"] = subOnlyQualifiers.apply(
    lambda x: "Sub only", axis=1
)
...
goldOnlyQualifiers["matchType"] = goldOnlyQualifiers.apply(
    lambda x: "Gold only", axis=1
)
...
subOnlyHasQuant["matchType"] = subOnlyHasQuant.apply(
    lambda x: "Sub only", axis=1
)
...
goldOnlyHasQuant["matchType"] = goldOnlyHasQuant.apply(
    lambda x: "Gold only", axis=1
)
...
goldOnlyHasProp["matchType"] = goldOnlyHasProp.apply(
    lambda x: "Gold only", axis=1
)
...
subOnlyQualifies["matchType"] = subOnlyQualifies.apply(
    lambda x: "Sub only", axis=1
)
...
goldOnlyQualifies["matchType"] = goldOnlyQualifies.apply(
    lambda x: "Gold only", axis=1
)
...
wrk1array = [
    quantityMatches[["docId", "matchType", "annotType", "EM", "maxF1"]].rename(
        columns={"maxF1": "F1", "annotType": "type"}
    ),
    subOnlyQuants[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    goldOnlyQuants[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    unitMatches[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    subOnlyUnits[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    goldOnlyUnits[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    entityMatches[["docId", "matchType", "annotType", "EM", "maxF1"]].rename(
        columns={"maxF1": "F1", "annotType": "type"}
    ),
    subOnlyEntities[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    goldOnlyEntities[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    propertyMatches[["docId", "matchType", "annotType", "EM", "maxF1"]].rename(
        columns={"maxF1": "F1", "annotType": "type"}
    ),
    subOnlyProperties[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    goldOnlyProperties[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    qualifierMatches[["docId", "matchType", "annotType", "EM", "maxF1"]].rename(
        columns={"maxF1": "F1", "annotType": "type"}
    ),
    subOnlyQualifiers[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    goldOnlyQualifiers[["docId", "matchType", "annotType", "EM", "F1"]].rename(
        columns={"annotType": "type"}
    ),
    hasQuantMatch[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    subOnlyHasQuant[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    goldOnlyHasQuant[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    hasPropMatch[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    subOnlyHasProp[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    goldOnlyHasProp[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    qualifiesMatch[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    subOnlyQualifies[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    goldOnlyQualifies[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    modsMatches[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    subOnlyMods[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
    goldOnlyMods[["docId", "matchType", "relType", "EM", "F1"]].rename(
        columns={"relType": "type"}
    ),
]
...
wrk1score["subject"] = wrk1score.apply(
    lambda x: cats[x["docId"].split("-")[0]], axis=1
)
...
# But with micro-averages at the class level.
    ...
    for annotType in [
        ...
        tp = len(
            wrk1score.loc[
                (
                    (wrk1score["matchType"] == "Match")
                    & (wrk1score["type"] == annotType)
                )
            ].index
        )
        fp = len(
        ...
        if tp + fp == 0:
            ...
            print(
                "Submission has no matches against gold data for " + annotType
            )
        else:
            print(
                "True positives (matching rows) for "
                + annotType
                + ": "
                + str(tp)
            )
            print(
                "False positives (submission only) for "
                + annotType
                + ": "
                + str(fp)
            )
            print(
                "False negatives (gold only) for " + annotType + ": " + str(fn)
            )
            print("")
            precision = tp / (tp + fp)
    ...
    for subject in [
        ...
        if tp + fp == 0:
            ...
            print(
                "True positives (matching rows) for " + subject + ": " + str(tp)
            )
            print(
                "False positives (submission only) for "
                + subject
